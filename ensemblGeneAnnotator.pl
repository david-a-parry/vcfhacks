...
GetOptions ("input=s", \$vcf, "directory=s" => \$genedir, "output=s" => \$out, "separate" => \$separate, "functional" => \$functional, "classes=s{,}" => \@classes, "add=s{,}" => \@add, "PREPARE" => \$prep, "DOWNLOAD_NEW" => \$downdb, "REPAIR" => \$repair, "help" => \$help, "manual" => \$man) or pod2usage(-message => "Syntax error", -exitval => 2);
pod2usage (-verbose => 2) if $man;
pod2usage (-verbose => 1) if $help;
pod2usage(-exitval => 2, -message => "--input is required") if not $vcf and not $downdb and not $prep and not $repair;
...
my $pre_progressbar = Term::ProgressBar->new({name => "Parsing database", count => keys(%database) + 1, ETA => "linear", });
my $next_update = 0;
my $pre_progress = 0;
...
LINE: while (my $line = $vcf_obj->readLine){
    ...
    if (not keys %annot){
        ...
        foreach my $entrez_id (keys %annot){
                ...
                my $i = binSearchLineWithIndex($entrez_id, $database{human_summary}->{fh}, $database{human_summary}->{idx}, $database{human_summary}->{length}, $database{human_summary}->{col});
                if ($i > 0){
                        chomp(my @humsum_line =  split ("\t", line_with_index($database{human_summary}->{fh}, $database{human_summary}->{idx}, $i)));
                        ...
                        if ($mim_accession ne "-"){
                                my $j = binSearchLineWithIndex($mim_accession, $database{mim_morbid}->{fh}, $database{mim_morbid}->{idx}, $database{mim_morbid}->{length}, $database{mim_morbid}->{col}, $database{mim_morbid}->{delimiter});
                                if ($j > 0){
MIM_UP:                             for (my $line_no = $j; $line_no > 0; $line_no--){
                                            my @mim_line = split (/\|/, line_with_index($database{mim_morbid}->{fh}, $database{mim_morbid}->{idx}, $line_no));
                                            if ($mim_line[2] eq $mim_accession){
                                                        push (@{$annot{$entrez_id}->{mim}}, $mim_line[0]);
                                                }else{
                                                        last MIM_UP;
                                                }
                                        }
MIM_DOWN:                               for (my $line_no = $j+1; $line_no <= $database{mim_morbid}->{length}; $line_no++){
                                                my @mim_line = split (/\|/, line_with_index($database{mim_morbid}->{fh}, $database{mim_morbid}->{idx}, $line_no));
                                                if ($mim_line[2] eq $mim_accession){
                                                        push (@{$annot{$entrez_id}->{mim}}, $mim_line[0]);
                                                }else{
...
sub prepare_database{
    ...
    my $dir = getcwd();
        ...
        if ($mode eq "replace"){
                ...
                if (@files < 1){
                        if ($oldest){
                                display_error_and_continue("No files missing in database.", "Oldest file is $oldest days old. Use the \"Update Database\" option to update.");
                        }else{
                                display_error_and_continue("Database OK", "No files missing in database.");
                        }
                        return 0;
                }
        }else{
    ...
    my $progressbar = Term::ProgressBar->new({name => "Prep Database", count => 100, ETA => "linear", });
        ...
        $next_update = $progressbar->update($db_percent) if $db_percent >= $next_update;
                ...
                if($file->{file} =~ /\.gz$/){
                        ...
                        if ($file->{file} =~ /\.ags/){ #make sure we use binmode for .ags files
                               #for some reason gzipped ASN1 file seems to give premature EOF even in binmode, so use MultiStream option
                                my $z = new IO::Uncompress::Gunzip $file->{file}, MultiStream => 1 || restore_file($file_exists, $file)
                                && display_error_and_exit("Error decompressing file", "Error decompressing $file->{file}") ;
                                open (my $ZOUT, ">$output") || restore_file($file_exists, $file) && display_error_and_exit("Error decompressing file", "Error decompressing $file->{file}");
                                binmode $ZOUT;
                                $z -> binmode;
                                my $buffer;
                                while ($z->read($buffer)){
            ...
            $next_update = $progressbar->update($db_percent) if $db_percent >= $next_update;
                ...
                if ($file->{file} =~ /HMD_Human5\.rpt/){
                        $increment /= 3;
                        move($file->{file}, "$file->{file}.bak") || display_error_and_exit("File Error", "Error creating file backup of $file", "Check permissions and/or disk space.") ;
                        open (my $HMD_DOWN, "$file->{file}.bak") || display_error_and_exit( "File Read Error", "Can't open $file->{file}.bak to read");
                        open (my $HMD_MOD, ">$file->{file}") || display_error_and_exit( "Write Error", "Can't open $file->{file} to write.");
...
sub sort_and_index_gene_files{
        my ($file, $sort_column, $prog_ref, $increment, $progressbar, $next_update,  $delimiter) = @_; #column no. is 0 based
        open (my $FILE, $file) || die "Can't open $file for sorting and indexing!\n";
    ...
    my $prev_counter = 0; 
        while (<$FILE>){
                ...
                chomp;
                if ((split /$delimiter/)[$sort_column]){ #some MGI files have empty columns
    ...
    $prev_counter = 0; 
        ...
        open (my $INDEX, "+>$indexfile") || die "can't open $indexfile for writing index ";
        binmode $INDEX;
        ...
        while (my ($gene, $genestructure, $uncaptured)  = $io->next_seq){
                ...
                foreach my $anno (@annotations){
                        if ($anno->tagname eq 'dblink'){
                                if ( ( exists $anno->{database}) && ( $anno->{database} eq 'generif' ) ){
                                        push (@rifs, $anno->{comment}->{text}) if defined $anno->{comment}->{text};
                                }
                ...
                foreach my $anno (@annotations){
                        next if ( ( defined $anno->term->authority ) && ($anno->term->authority eq 'STS marker') ) ; # No STS markers
...
sub get_MGI_phenotype{
        my ($gene_id, $PHENO, $PHENO_INDEX, $pheno_line_count, $PHENO_DESC,  $PHENO_DESC_INDEX, $pheno_desc_line_count, $MGI_ENTREZ, $MGI_ENTREZ_INDEX, $mgi_entrez_line_count) = @_;
        my @desc = ();
        my $i = binSearchLineWithIndex($gene_id, $MGI_ENTREZ, $MGI_ENTREZ_INDEX, $mgi_entrez_line_count, 8);
        if ($i > 0){
                my $accession = (split "\t", line_with_index($MGI_ENTREZ, $MGI_ENTREZ_INDEX, $i))[0];
                my $j = binSearchLineWithIndex($accession, $PHENO, $PHENO_INDEX, $pheno_line_count, 5);
                if ($j > 0){
                        my @phenos = split(",", (split "\t", line_with_index($PHENO, $PHENO_INDEX, $j))[9]);
                        if (@phenos){
                                foreach my $ph (@phenos){
                                        my $k = binSearchLineWithIndex($ph, $PHENO_DESC,  $PHENO_DESC_INDEX, $pheno_desc_line_count, 0);
                                        if ($k > 0){
                                                push (@desc, (split "\t", line_with_index($PHENO_DESC, $PHENO_DESC_INDEX, $k))[1]);
                                        }else{
                                                display_error_and_continue("Missing phenotype description", "Can't find matching phenotype description for phenotype ID $ph\nTry updating your database.");
                                        }
                                }
                        }
                }
