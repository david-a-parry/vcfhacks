...
filterOnSample - filter variants in vcf that belong to specific samples.

=cut

=head1 SYNOPSIS

    filterOnSample --input [var.vcf] --samples [samples to keep variants if present in all] --reject [samples to reject variants from if present in any] 
...
=head1 EXAMPLES
 ...
 filterOnSample -i [var.vcf] -s Sample1
 (only print variants if Sample1's genotype has a variant allele)

 filterOnSample -i [var.vcf] -s Sample1 -r Sample2 Sample3
 (look for variants in Sample1 but reject variants also present in Sample2 or Sample3)
 
 filterOnSample -i [var.vcf] -s Sample1 -r Sample2 Sample3 -a 20 -u 30
