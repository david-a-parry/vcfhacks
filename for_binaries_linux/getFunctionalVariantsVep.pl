...
unless ($no_head){
    print $OUT  $vcf_obj->getHeader(0) ."##getFunctionalVariantsVep=\"";
    my @opt_string = ();
...
getFunctionalVariantsVep  -  retrieve specific variant classes from an annotated vcf file.

=head1 SYNOPSIS

=over

=item getFunctionalVariantsVep --input <annotated vcf file> [options]

=item getFunctionalVariantsVep --help for help message

=item getFunctionalVariantsVep --manual for manual page
...
=item getFunctionalVariantsVep -i var.vcf 

(filter on default set of consequences, print output to STDOUT.)


=item getFunctionalVariantsVep -i var.vcf --classes stop_gained stop_lost

(filter on 'stop_gained' and 'stop_lost' classes only.)


=item getFunctionalVariantsVep -i var.vcf --maf 0.01 

(filter on default set of consequences, filter anything with a MAF greater than or equal to 0.01 [1 %].)


=item getFunctionalVariantsVep -i var.vcf --maf 0.01 --damaging all 

(As above but only keep missense variation predicted damaging by all annotation programs.)


=item getFunctionalVariantsVep -i var.vcf --maf 0.01 --damaging all --list gene_list.txt 

(As above but also filter genes in gene_list.txt file.)


=item getFunctionalVariantsVep -i var.vcf --maf 0.01 --damaging all --list gene_list.txt -o output.vcf
