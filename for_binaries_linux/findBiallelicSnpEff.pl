...
print $OUT $vcf_obj->getHeader(0) . "##findBiallelicSnpEff\"";
my @opt_string = ();
...
print $OUT
"##INFO=<ID=findBiallelicSnpEffSamplesHom,Number=A,Type=String,Description=\"For each allele a list of samples that were found to meet findBiallelicSnpEff's criteria for constituting a homozygous variant.\">\n";
print $OUT
"##INFO=<ID=findBiallelicSnpEffSamplesHet,Number=A,Type=String,Description=\"For each allele a list of samples that were found to meet findBiallelicSnpEff's criteria for constituting a potential compound heterozygous variant.\">\n";
...
findBiallelicSnpEff - identify variants that make up potential biallelic variation of a gene using snpEff annotations.

=head1 SYNOPSIS

    findBiallelicSnpEff -i <snpeff.variants.vcf> -s <sample1> <sample2> [options]
    findBiallelicSnpEff --help (show help message)
    findBiallelicSnpEff --manual (show manual page)
...
=head1 EXAMPLES
    ...
    findBiallelicSnpEff -i <variants.vcf> -s <sample1> <sample2> -r <sample3> <sample4>  -o output.vcf -l genelist.txt
    #find genes with biallelic variants in two unrelated samples but not in two unaffected samples. 

    findBiallelicSnpEff -i <variants.vcf> -s <sample1> <sample2> -e -o output.vcf -l genelist.txt
    #find genes with biallelic variants in two related samples where you expect them to share the same causative variant.

    findBiallelicSnpEff -i <variants.vcf> -f families.ped -o output.vcf -l genelist.txt -q 30
