...
unless ($no_head){
    print $OUT  $meta_header ."##getFunctionalVariantsSnpEff=\"";
    my @opt_string = ();
...
getFunctionalVariantsSnpEff  -  retrieve specific variant classes from a SnpEff annotated vcf file.

=head1 SYNOPSIS

=over

=item getFunctionalVariantsSnpEff --input <annotated vcf file> [options]

=item getFunctionalVariantsSnpEff --help for help message

=item getFunctionalVariantsSnpEff --manual for manual page
...
=item getFunctionalVariantsSnpEff -i var.vcf 

(filter on default set of consequences, print output to STDOUT)


=item getFunctionalVariantsSnpEff -i var.vcf --classes STOP_LOST STOP_GAINED

(keep variants with 'STOP_GAINED' or 'STOP_LOST' classes only)


=item getFunctionalVariantsSnpEff -i var.vcf --1000_genomes_allele_frequency 0.01 
...
=item getFunctionalVariantsSnpEff -i var.vcf --1000_genomes_allele_frequency 0.01 --damaging all 

(As above but only keep missense variation predicted damaging by all annotation programs)


=item getFunctionalVariantsSnpEff -i var.vcf --1000_genomes_allele_frequency 0.01 --damaging all --list gene_list.txt 

(As above but also filter genes in gene_list.txt file)


=item getFunctionalVariantsSnpEff -i var.vcf --1000_genomes_allele_frequency 0.01 --damaging all --list gene_list.txt -o output.vcf

(As above but specifying output file)

=item getFunctionalVariantsSnpEff -i var.vcf --allele_frequency 0.01 --damaging all --list gene_list.txt -o output.vcf

(Filtering if any given population specific or global allele frequency is greater than or equal to 0.01)

=item getFunctionalVariantsSnpEff -i var.vcf o output.vcf --filter_dbnsfp "dbNSFP_GERP++_RSE<lt>2"

(Filter anything with a GERP RS score below 2)

=item getFunctionalVariantsSnpEff -i var.vcf -o output.vcf --filter_dbnsfp "dbNSFP_GERP++_RSE<lt>2" --match_dbnsfp "dbNSFP_Interpro_domain=GPCR"
