...
ensemblGeneAnnotator - add gene annotations to a VCF file annotated by Ensembl's variant_effect_predictor.pl

=head1 SYNOPSIS

        ensemblGeneAnnotator -i [VEP annotated vcf file] -d [directory containing reference files] [options]
        ensemblGeneAnnotator -h (display help message)
        ensemblGeneAnnotator -m (display manual page)
...
=head1 EXAMPLES 
        ...
        ensemblGeneAnnotator -i vep_annotated.vcf -d ~/ensGeneAnnotatorDatabase -o vep_and_ensGene_annotated.vcf
        #annotate vep_annotated.vcf file using database folder located at ~/ensGeneAnnotatorDatabase
        
        ensemblGeneAnnotator -i vep_annotated.vcf -d ~/ensGeneAnnotatorDatabase -o vep_and_ensGene_annotated.vcf --functional
        #as above but only considering genes which have at least one 'functional' variant consequence
        
        ensemblGeneAnnotator -d ~/newEnsGeneAnnotatorDatabase -D
        #download files and create database in ~/newEnsGeneAnnotatorDatabase folder

        ensemblGeneAnnotator -d ~/newEnsGeneAnnotatorDatabase -R
