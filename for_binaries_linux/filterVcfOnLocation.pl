...
filterVcfOnLocation - print variants from a vcf file that lie within a list of genomic regions without requiring sorting of your VCF


=head1 SYNOPSIS
 
filterVcfOnLocation -i [vars.vcf] -b [regions.bed] -o [output.vcf]
...
Takes a bed file or a list of regions and filters variants from a vcf file that lie in these regions. For sorted VCF files it is recommended to use getVariantsByLocation.pl instead if you will be searching the same VCF repeatedly. This program (filterVcfOnLocation) should be used if you want to search an unsorted VCF.
