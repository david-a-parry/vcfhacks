...
print $OUT $vcf_obj->getHeader(0) . "##findBiallelicVep\"";
my @opt_string = ();
...
print $OUT
"##INFO=<ID=findBiallelicVepSamplesHom,Number=A,Type=String,Description=\"For each allele a list of samples that were found to meet findBiallelicVep's criteria for constituting a homozygous variant.\">\n";
print $OUT
"##INFO=<ID=findBiallelicVepSamplesHet,Number=A,Type=String,Description=\"For each allele a list of samples that were found to meet findBiallelicVep's criteria for constituting a potential compound heterozygous variant.\">\n";
...
findBiallelicVep - identify variants that make up potential biallelic variation of a gene

=head1 SYNOPSIS

    findBiallelicVep -i <variants.vcf> -s <sample1> <sample2> [options]
    findBiallelicVep --help (show help message)
    findBiallelicVep --manual (show manual page)
...
=head1 EXAMPLES
    ...
    findBiallelicVep -i <variants.vcf> -s <sample1> <sample2> -r <sample3> <sample4>  -o output.vcf -l genelist.txt
    #find genes with biallelic variants in two unrelated samples but not in two unaffected samples. 

    findBiallelicVep -i <variants.vcf> -s <sample1> <sample2> -r <sample3> <sample4> -d polyphen --maf 0.01 -o output.vcf -l genelist.txt
    ...
    findBiallelicVep -i <variants.vcf> -s <sample1> <sample2> -e -o output.vcf -l genelist.txt
    #find genes with biallelic variants in two related samples where you expect them to share the same causative variant.

    findBiallelicVep -i <variants.vcf> -f families.ped -o output.vcf -l genelist.txt -q 30
    ...
    findBiallelicVep -i <variants.vcf> -f families.ped -o output.vcf -l genelist.txt -w 10 -z 30
