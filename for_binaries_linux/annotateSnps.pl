...
print $OUT "##annotateSnps=\"";
print $KNOWN "##annotateSnps=\"" if $KNOWN;
...
annotateSnps - annotate and optionally filter SNPs from a VCF file 

=head1 SYNOPSIS

        annotateSnps -i [vcf file] -d [reference SNP vcf file] [options]
        annotateSnps -h (display help message)
        annotateSnps -m (display manual page)
...
annotateSnps -d dbSnp138.b37.vcf.gz clinvar_20130506.vcf -b 129 -f 1 --pathogenic -i input.vcf -o input_filtered.vcf
