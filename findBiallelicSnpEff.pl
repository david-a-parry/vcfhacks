...
LINE: while (my $line = $vcf_obj->readLine){
    ...
    my @csq = $vcf_obj->getSnpEffFields(\@csq_fields); #returns array of hashes e.g. $csq[0]->{Gene} = 'ENSG000012345' ; $csq[0]->{Consequence} = 'missense_variant'
...
EFFECT:  foreach my $impact (@effect_impact){
        ...
        if ($matches_class){
            ...
            if ($annot->{Gene_Name}){
                ...
                $id_to_symbol{$annot->{Transcript_ID}} = $annot->{Transcript_ID};
            }
        }
    }
}
